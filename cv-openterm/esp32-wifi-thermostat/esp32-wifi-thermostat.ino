...
void setup(void) {
    ...
    server.on("/test", []() { server.send(200, "text/plain", "this works as well"); });
...
void processResponse(unsigned long response, OpenThermResponseStatus status) {
    ...
    switch (id) {
        ...
        case OpenThermMessageID::RelModLevel:
            ...
            break;
        default: Serial.println("Response: " + String(response, HEX) + ", id=" + String(id));
    }
}
...
unsigned int buildRequest(byte req_idx) {
    ...
    switch (id) {
        case OpenThermMessageID::Status:
            status = 0;
            if (CHEnabled)
                status |= MASTER_STATUS_CH_ENABLED;
            if (DHWEnabled)
                status |= MASTER_STATUS_DHW_ENABLED;
            if (CoolingEnabled)
                status |= MASTER_STATUS_COOLING_ENABLED;
            status <<= 8;
            return ot.buildRequest(OpenThermMessageType::READ, OpenThermMessageID::Status, status);
        case OpenThermMessageID::TSet: return ot.buildRequest(OpenThermMessageType::WRITE, OpenThermMessageID::TSet, ((uint16_t)ch_setpoint) << 8);
        case OpenThermMessageID::Tboiler: return ot.buildRequest(OpenThermMessageType::READ, OpenThermMessageID::Tboiler, 0);
        case OpenThermMessageID::RelModLevel: return ot.buildRequest(OpenThermMessageType::READ, OpenThermMessageID::RelModLevel, 0);
    }
    return 0;
}
