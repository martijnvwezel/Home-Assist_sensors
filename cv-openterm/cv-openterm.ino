...
float getTemp() {
    ...
    if (now - lastTempSet > extTempTimeout_ms)
        return -1.0; // sensors.getTempCByIndex(0);
    else
        return t;
}
...
void updateData() {
    ...
    String tempSource = (millis() - lastTempSet > extTempTimeout_ms) ? "(internal sensor)" : "(external sensor)";
...
bool isValidNumber(String str) {
    ...
    for (byte i = 0; i < str.length(); i++) {
        ...
        valid &= isDigit(ch) || ch == '+' || ch == '-' || ch == ',' || ch == '.' || ch == '\r' || ch == '\n';
    }
    return valid;
}
