...
bool convert_image(uint8_t *src, uint16_t width, uint16_t height, pixformat_t format, uint8_t quality, uint8_t *dst_stream)
{
    int num_channels = 3;
    ...
    if (!dst_image.init(dst_stream, width, height, num_channels, comp_params)) {
        // Serial.print(TAG, "JPG encoder init failed");
        return false;
    }

    uint8_t* line = (uint8_t*)malloc(width * num_channels);
    if(!line) {
        // Serial.print(TAG, "Scan line malloc failed");
        return false;
    }
    ...
    for (int i = 0; i < height; i++) {
        ...
        if (!dst_image.process_scanline(line)) {
            // Serial.print(TAG, "JPG process line %u failed", i);
            free(line);
            return false;
        }
    }
    ...
    if (!dst_image.process_scanline(NULL)) {
        // Serial.print(TAG, "JPG image finish failed");
        return false;
    }
    dst_image.deinit();
    return true;
