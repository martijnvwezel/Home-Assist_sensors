...
bool fmt_2_jpg(uint8_t *src, size_t src_len, uint16_t width, uint16_t height, pixformat_t format, uint8_t quality, uint8_t ** out, size_t * out_len) {
    ...
    convert_image(src, width, height, format, quality, jpg_buf);
}
...
static esp_err_t stream_handler(httpd_req_t* req) {
    ...
    if (res != ESP_OK) {
        Serial.print("1Res esp: "); Serial.println(res);
        return res;
    }
    ...
    while (true) {
        ...
        // void * bliem = malloc(64);
        // if(bliem) {
        //     Serial.println("malloc went success: ");
        // }
        bool jpeg_converted = true;
        _jpg_buf = fb.buf;
        _jpg_buf_len = fb.len;
        if (res == ESP_OK) {
            size_t hlen = snprintf((char*)part_buf, 64, _STREAM_PART, _jpg_buf_len);
            res         = httpd_resp_send_chunk(req, (const char*)part_buf, hlen);
            Serial.print("2Res esp: "); Serial.println(res);
        }
        if (res == ESP_OK) {
            res = httpd_resp_send_chunk(req, (const char*)_jpg_buf, _jpg_buf_len);
            Serial.print("3Res esp: "); Serial.println(res);
        }
        if (res == ESP_OK) {
            res = httpd_resp_send_chunk(req, _STREAM_BOUNDARY, strlen(_STREAM_BOUNDARY));
            Serial.print("4Res esp: "); Serial.println(res);
        }
