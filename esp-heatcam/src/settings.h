...
bool convert_image(uint8_t *src, uint16_t width, uint16_t height, pixformat_t format, uint8_t quality, jpge::output_stream *dst_stream)
{
    ...
    if (!dst_image.init(dst_stream, width, height, num_channels, comp_params)) {
        Serial.print( "JPG encoder init failed");
        return false;
    }

    uint8_t* line = (uint8_t*)malloc(width * num_channels);
    if(!line) {
        Serial.print( "Scan line malloc failed");
        return false;
    }
    ...
    for (int i = 0; i < height; i++) {
        ...
        if (!dst_image.process_scanline(line)) {
            Serial.print("JPG process line failed: ");Serial.println(i);
            free(line);
            return false;
        }
    }
    ...
    if (!dst_image.process_scanline(NULL)) {
        Serial.println("JPG image finish failed");
        return false;
    }
    dst_image.deinit();
    return true;
